...
class RegistroRepository extends ServiceEntityRepository
{
    ...
    */
    ...
    public function findAllRegistroByEnteCategoria(int $enteId, int $categoriaId)
    {
    ...
    */
        ...
        i.categoria_id,
        i.id  as indicador_id,    
        ifnull(r.registro_id,0) as registro_id,
    ...
    /*public function cnedContextoInstitucional(int $enteId, int $categoriaId, int $periodoId)
    {
        $conn = $this->getEntityManager()->getConnection();
        $sql =  'call `indicadores-cft`.`SP_CNED_Contexto_Insitucional`(:enteId,:categoriaId,:periodoId)';
        $stmt = $conn->prepare($sql);
        $resultSet = $stmt->executeQuery(['enteId' => $enteId, 'categoriaId' => $categoriaId, 'periodoId' => $periodoId]);
