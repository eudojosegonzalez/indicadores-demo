...
class ReporteController extends AbstractController
{
    ...
    public function searchIndicadores(
        ...
        if ($indicadores) {
            ...
            $sheet->setCellValue('E6', 'Resultado');
            $i = 7;
            $u = 1;
            foreach ($indicadores as $indicador) {
                $sheet->setCellValue('A' . $i, $u);
                $sheet->setCellValue('B' . $i, $indicador['concepto']);
                $sheet->setCellValue('C' . $i, $indicador['Definición']);
                $sheet->setCellValue('D' . $i, $indicador['Fórmula']);
                $sheet->setCellValue('E' . $i, $indicador['valor']);
                $i++;
                $u++;
            }
